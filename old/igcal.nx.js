...
function parseCalendarXML__MODULE_ID__(feedXML){
  var responseDOM = xmlParse(feedXML);
  var nxsl = xmlParse('< display=false select="/feed/entry[1]/author[1]" values="over:."');
 nxslProcess(responseDOM, nxsl);
  alert(xmlText(nxsl));
...
alert(responseDom);
  ...
  var calTitle = shrink__MODULE_ID__(responseDOM.getElementsByTagName("title")[0].getText(),20,'');
